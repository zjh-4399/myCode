function InfluenceGeoGaussian_20260108_zjh( zs, alpha, RunType, Dalpha, deltas  )
...
%       sigma( ii ) = min( 0.2 * freq * ray( is ).tau / lambda , pi * lambda );
      ii =  sigma < min( 2.0 * freq * ray( is ).tau , 2*pi * lambda ) ; %zb2\zb3=0.6,2*pi*lambda,zb4=0.8,2*pi*lambda,zb5=1.0,2*pi*lambda
      sigma( ii ) = min( 2.0 * freq * ray( is ).tau , 2*pi * lambda );  %zb2=2.0,2*pi*lambda
