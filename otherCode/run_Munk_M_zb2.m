...
for num = 1:length(freqList)
    FreqDir = ['/workspace/zjh/Result_zb6/' num2str(freqList(num)) 'Hz/'];
...
tic
for ifreq = 8 %:length(freqList)
FreqDir = ['/workspace/zjh/Result_zb6/' num2str(freqList(ifreq)) 'Hz/'];
...
%
Nbeams = max( ceil( 0.6 * 1000 * rmax * f0 / 1500 ), 300 );   % automatically estimate NBeams to use
