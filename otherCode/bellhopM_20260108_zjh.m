function bellhopM_20260108_zjh( filename )
...
[ PlotTitle, freq, SSP, Bdry, Pos, Beam, ~, ~, fid ] = read_env_20260108_zjh( envfil, 'BELLHOP' );    % read in the environmental file
       ...
       if ( mod( ibeam - 1, max( fix( Nbeams / 50 ), 1 ) ) == 0 )
        ...
        if Beam.RunType(1:1) == 'R'
            ...
            if ( Beam.RunType(2:2) == 'G' )
                ...
                InfluenceGeoGaussian_20260108_zjh( Pos.s.z( is ), alpha( ibeam ), Beam.RunType, Dalpha, Beam.deltas  )
            end
        end
    end
