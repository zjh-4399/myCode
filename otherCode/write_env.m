...
if ( Bdry.Top.Opt( 2:2 ) == 'A' )
    fprintf( fid, '    %6.4f %6.4f %6.4f %6.4g %6.4f %6.4f /  \t ! upper halfspace \n', SSP.depth( 1 ), ...
...
for medium = 1 : SSP.NMedia
    ...
    fprintf( fid, '%5i %4.4f %6.4f \t ! N sigma depth \n', SSP.N( medium ), SSP.sigma( medium ), SSP.depth( medium+1 ) );
    for ii = 1 : length( SSP.raw( medium ).z )
        fprintf( fid, '\t %6.4f %6.4f %6.4f %6.4g %10.6f %6.2f / \t ! z c cs rho \n', ...
            [ SSP.raw( medium ).z( ii ) ...
...
% lower halfspace
fprintf( fid, '''%s'' %6.4f  \t \t ! Bottom Option, sigma \n', Bdry.Bot.Opt, 0.0 ); % SSP.sigma( 2 ) );

if ( Bdry.Bot.Opt( 1:1 ) == 'A' )
    fprintf( fid, '    %6.4f %6.4f %6.4f %6.4g %6.4f %6.4f /  \t ! lower halfspace \n', SSP.depth( SSP.NMedia+1 ), ...
