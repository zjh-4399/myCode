...
for num = 1:length(freqList)
    FreqDir = ['/workspace/zjh/Result_M1/' num2str(freqList(num)) 'Hz/'];
...
FreqDir = ['/workspace/zjh/Result_M1/' num2str(freqList(ifreq)) 'Hz/'];
...
envname = 'MunkB_CB_M1'; % env文件名，可自定义
...
%
Nbeams = max( ceil( 0.3 * 1000 * rmax * f0 / 1500 ), 300 );   % automatically estimate NBeams to use
d_theta_recommended = atan(max(zw)/(10 * 1000 * rmax));
Nbeams = max( ceil( 2* pi / d_theta_recommended ), Nbeams );
%
nbeams = Nbeams; % 声线数选择，设为0的话程序会自动计算需要的声线根数
